...
def eval_with_LIME(proj_name, clf, explainer, test_features, iteration):
    ...
    #print("True buggy commits:",len(correctly_predicted_commit))
    ...
    for commit in correctly_predicted_commit:
        ...
        #print(type(code_change_from_line_level_df),len(code_change_from_line_level_df))
        ...
        if len(code_change_from_line_level_df) > 0:
            ...
            #print("test_features:",len(test_features.columns))
            #print(test_features.columns) # #The shape error that took forever. It occurred because lime was trained on different features than it was being evaluated on
            #print('Here:',(test_features.loc[commit])) #If you can't find it then you're probably reading more from prediction file and correctly_predicted_commit have extra commits
...
def run_experiment(cur_proj,number_of_splits,eval_size):
    ...
    for eval_set in evaluation_set:
        ...
        #test_df.to_csv(data_path + cur_proj + "_sampled_test_commits_RQ4.csv",mode='a',header=False,index=False)

        #line_level_result = eval_line_level(cur_proj, k_values_npm[iteration-1], df, train_data, test_data, iteration, CLH_queue,type3_dict)
