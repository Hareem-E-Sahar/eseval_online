...
def get_combined_df(code_commit, commit_id, label, metrics_df, count_vect):
    ...
    #print("code_df.columns:",len(code_df.columns)) #len=3 
    #print(code_df.columns)			     #commit_id, code, label
    metrics_df = metrics_df.sort_values(by='commit_id')
    metrics_df = metrics_df.drop('commit_id',axis=1)
    #print("metrics_df.columns:",len(metrics_df.columns)) #len=12
    #print(metrics_df.columns)	 	 #'ns', 'nd', 'nf', 'entrophy', 'la', 'ld', 'ndev', 'age', 'nuc', 'exp','rexp', 'sexp'
...
def check_queue_inplace(WFL_queue,CLH_queue,current_timestamp,type3_dict):
    ...
    #well-suited for our batch processing scenario
...
'''
def check_queue(WFL_queue,T):
    #Input = examples between two test commits
    #assume an empty set and add to it
    #add example if it has a defect reported
    #or add examples that are proven clean
    #This implementation is well-suited for a real
    #online scenario, but not for a batch processing
    #scenario that we simulated for JITLine.

    tr_examples=[]
    if len(WFL_queue) > 0:
        for element in WFL_queue:
            tr_timestamp = element[14]
            if ((element in t3_dict) and tr_timestamp < T) :
                tr_examples.append(element)
            if calculate_diff(T-tr_timestamp)>=w:
                tr_examples.append(element) #we use it as a clean Tr example
                put_in_CLH_queue('clean')
    return tr_examples              #new examples to be added to the existing Tr set
'''
...
def main():
    ...
    #openstack_k_of_smote, openstack_best_AUC_of_obj_func = run_experiment('spring-integration')
    ...
    #eval_result('spring-integration')
    #eval_result('qt')

    # ## RQ3 result
    # They should not pass the best k_neighbors as input because it comes from
    # differential_evolution and that is not run here which means the time to
    # find k_neighbors is not been taken into account.
    #check_train_time('spring-integration', 9)
